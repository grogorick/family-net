...
function AutoLayout1()
{
  this.apply = () => {
    if (data.graph.persons.length) {
      ...
      this.prepare(p0);
      console.log('layout');
      let fullWidth = this.layout(p0);

      //for (let i = 1; i < fullWidth; ++i) {
      //  [-1, 1].forEach(xfactor => {
      //    let ii = xfactor * i;
      //    let x = settings_1.nodeSpacingX * ii;
  ...
  this.hideAll = () => {
    // s.graph.nodes().forEach(n => n.hidden = true);
    // s.graph.edges().forEach(e => e.hidden = true);
  ...
  this.prepare = (p) => {
    p.al1 = { size: { left: 0, right: 0 } };
    switch (p._parents.length) {
    case 0: break;
    case 1:
      let pp = p._parents[0].p;
      this.prepare(pp);
      p.al1.size.left = pp.al1.size.left;
      p.al1.size.right = pp.al1.size.right;
      break;
    case 2:
      p._parents.forEach(pp => this.prepare(pp.p));
      let pp1 = p._parents[0].p;
      let pp2 = p._parents[1].p;
      p.al1.size.left = pp1.al1.size.left + pp1.al1.size.right + settings_1.nodeSpacingX;
      p.al1.size.right = pp2.al1.size.left + pp2.al1.size.right + settings_1.nodeSpacingX;
      break;
    }
  };
  ...
  this.layout = (p) => {
    ...
    case 2:
      ...
      let distance = pp1.p.al1.size.right + pp2.p.al1.size.left + 2 * settings_1.nodeSpacingX;
      pp1.pc._graphEdge.label = distance.toString();
      pp1.p._graphNode.x = x - pp1.p.al1.size.right - settings_1.nodeSpacingX;
      pp1.p._graphNode.y = y;

      pp2.p._graphNode.x = x + pp2.p.al1.size.left + settings_1.nodeSpacingX;
    ...
    {
      pp.p._graphNode.label += '\n' + pp.p.al1.size.left + '/' + pp.p.al1.size.right + '\n' + pp.p._graphNode.x;
